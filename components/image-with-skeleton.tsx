 ...
 * enunciado, base del diagram, zona del graph_click, respuestas del alumno).
 *
...
export function ImageWithSkeleton({
  ...
  if (src === failedSrc) {
    return (
      <div
        className={cn(
          "flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed bg-muted/30 p-6 text-xs text-muted-foreground",
          skeletonClassName
        )}
      >
  ...
  return (
    <>
      {!isLoaded && (
        // `bg-accent` (el color por defecto de Skeleton) es casi blanco y no se
        // distingue de la tarjeta, así que aquí tiramos del gris de texto.
        <Skeleton
          className={cn(
            "flex h-48 w-full items-center justify-center rounded-lg bg-muted-foreground/15",
            skeletonClassName
          )}
        >
